<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Abbonamento attivo non trovato</title>
</head>
<body style="background-color: #f6f6f6; font-family: Arial, sans-serif; margin: 0; padding: 0;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f6f6f6; padding: 30px 0;">
        <tr>
            <td align="center">
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); padding: 32px;">
                    <tr>
                        <td>
                            <h2 style="color: #333; margin-top: 0;">Abbonamento attivo non trovato</h2>
                            <p style="color: #555; font-size: 16px;">
                                Gentile cliente,
                            </p>
                            <p style="color: #555; font-size: 16px;">
                                Grazie per averci contattato.
                            </p>
                            <p style="color: #555; font-size: 16px;">
                                Ti informiamo che la tua richiesta è attualmente in fase di valutazione. Il nostro team sta esaminando i dettagli e ti contatteremo al più presto con ulteriori informazioni.
                            </p>
                            <p style="color: #555; font-size: 16px;">
                                Ti ringraziamo per la pazienza e ti assicuriamo una risposta al più presto.
                            </p>
                            <p style="margin-top: 32px; color: #888; font-size: 15px;">
                                Cordiali saluti,<br>
                                <span style="color: #333;">Il team di assistenza</span>
                            </p>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>

    <div style="font-family: Arial, sans-serif; color: #333; font-size: 14px; line-height: 1.4; padding:30px;">
        {!! $SENDER_EMAIL_PREVIEW ?? '' !!}
    </div>
</body>
</html>